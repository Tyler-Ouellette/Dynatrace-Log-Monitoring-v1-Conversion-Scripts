...
## This script reads a prod.json which is an extract of the v1 Log Metrics screen, done by Michael Dec and his dev team
## It parses the metrics and gets the process and host names back via API.
## It then converts the existing configs into Log v2 compatible configurations and exports it to a csv.

filename = 'LOG_METRIC_REGISTRATION_Config.json'
url = ''
token = ''
...
def get_process(process):
    ...
    if process['type'] == "PROCESS_GROUP":
        req = requests.get(url + "/api/v1/entity/infrastructure/process-groups/PROCESS_GROUP-" + h, headers=headers, verify=False)
        j = json.loads(req.content)

    if process['type'] == "PROCESS_GROUP_INSTANCE":
        req = requests.get(url + "/api/v1/entity/infrastructure/processes/PROCESS_GROUP_INSTANCE-" + h, headers=headers, verify=False)
        j = json.loads(req.content)
...
def create_OS_query(found_filters):
    ...
    if len(path_query) == 2 or len(host_query) == 2:
        # print("path or hosts has a problem in OS metric")
        return ""
...
# create a Log v2 style query from original log metric details
def create_query(metric, found_filters):
    query = ""
    
    # create string search part of query
    if metric["searchString"]:
        query = query + create_search(metric["searchString"])
...
def generate_row(metric):
    found_filters = []
    for f in metric['logSourceFilters']:
        found_filters.append(find_filters(f))
        # print(found_filters)
        
    query = create_query(metric, found_filters)

    description = "Imported Log v1 metric." # Originally created by " + metric['auditInfo']['user']
    
    # print(metric)
    row = [metric['id'][5:], metric['searchString'], query, metric['metricValueType'], description] 
    # row = [metric['id'], metric['patternName'], metric['searchString'], query, metric['patternName'], description, "Error", found_filters[0]['version']] 
...
def main():
    ...
    for metric in data:
        logmetrics.append(generate_row(metric))

    data_columns = ['Log Metric Key', "Search String", 'Log Query', 'Type', 'Description']
    # data_columns = ['Log metric Orig ID', 'Summary', "Orig Search", 'Log Query', 'Title', 'Description', 'metric Type', 'Agent Version']
    df = pd.DataFrame(data = logmetrics, columns=data_columns)
    #print (df)
    
    df.to_csv("logv2metrics_" + filename.rstrip(".json") + ".csv", encoding='utf-8', index=False)
